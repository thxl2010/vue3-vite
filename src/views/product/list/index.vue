    ...
    <app-card>
      ...
      >
        ...
        <el-form-item label="商品类目">
          <el-radio-group v-model="listParams.type" @change="loadList">
...
const handleUpdateProductsShow = async () => {
  ...
  await productApi
    .updateProductsShow(selectionItems.value.map((item) => item.id))
    .finally(() => {
...
const handleUpdateProductsUnshow = async () => {
  ...
  await productApi
    .updateProductsUnshow(selectionItems.value.map((item) => item.id))
    .finally(() => {
